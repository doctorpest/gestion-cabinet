    ...
    >
      ...
      <template v-slot:bottom-row>
        <q-tr class="bg-grey-2 text-bold">
          ...
          <q-td class="text-right text-negative">{{ totalDu }} €</q-td>
          <q-td class="text-right text-positive">{{ totalPaye }} €</q-td>
          <q-td class="text-right text-warning">{{ totalImpayé }} €</q-td>
        </q-tr>
      </template>
    </q-table>
...
const goToCompta = async () => {
  await router.push('/assistante/comptabilite')
}
